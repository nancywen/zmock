...
import com.zf.service.FileHandler;
...
public class WebController {
	...
	public String mockIndex(String role, Map<String, Object> map) {
		...
		map.put("mockCurrentDataCount", DataProviderService.MOCK_DATAS.values()
				.size());
		return "mock/index";
	}

	@RequestMapping(value = "/content")
	public String mockContent(String collectionName, String mockName,
		...
		MockInfoDao rules = DataProviderService.MOCK_DATAS.get(collectionName
				+ DataProviderService.MOCK_DATAS_KEY_CONNECTOR + mockName);
		map.put("rules", rules);
	...
	public List<Map<String, Object>> getCollectionsTree() {
		...
		for (String key : DataProviderService.MOCK_COLLECTIONS.keySet()) {
			...
			for (String mockName : DataProviderService.MOCK_COLLECTIONS
					.get(key)) {
				Map<String, Object> mockMap = new LinkedHashMap<>();
				mockMap.put("text", mockName);
				mockMap.put("attributes", MapperBuilder
						.<String, String> getBuilder().put("pnode", key)
						.build());
	...
	@ResponseBody
	public ResponseUtil.ResponseInfo addMock(String collectionName,
			String mockName) {
		...
		if (!DataProviderService.MOCK_COLLECTIONS.containsKey(collectionName)) {
			DataProviderService.MOCK_COLLECTIONS.put(collectionName,
					new ArrayList<String>());
		}
		if (DataProviderService.MOCK_COLLECTIONS.get(collectionName).contains(
				mockName)) {
			ResponseUtil.ResponseInfo res = ResponseUtil.getFailedResponse();
		...
		DataProviderService.MOCK_DATAS.put(collectionName
				+ DataProviderService.MOCK_DATAS_KEY_CONNECTOR + mockName,
				new MockInfoDao());
	...
	public String edit(String collectionName, String mockName,
	...
	@ResponseBody
	public ResponseUtil.ResponseInfo editMock(String collectionName,
			String mockName, String oldCollectionName, String oldMockName) {
		if (StringUtils.isBlank(collectionName)) {
			ResponseUtil.ResponseInfo res = ResponseUtil.getFailedResponse();
			res.setRetMsg("请输入COLLECTION名称！");
			return res;
		}
		if (!collectionName.equals(oldCollectionName)
				&& StringUtils.isBlank(mockName)) {
			boolean flag = false;
			if (!DataProviderService.MOCK_COLLECTIONS
					.containsKey(collectionName)) {
				DataProviderService.MOCK_COLLECTIONS.put(collectionName,
						new ArrayList<String>());
				flag = true;
			}
			List<String> clist = DataProviderService.MOCK_COLLECTIONS
					.get(collectionName);
			List<String> oclist = DataProviderService.MOCK_COLLECTIONS
					.get(oldCollectionName);
			if (clist != null) {
				for (String s : clist) {
					if (oclist.contains(s)) {
						if (flag) {
							DataProviderService.MOCK_COLLECTIONS
									.remove(collectionName);
						}
						ResponseUtil.ResponseInfo res = ResponseUtil
								.getFailedResponse();
			...
			for (String s : oclist) {
				MockInfoDao oMockInfo = DataProviderService.MOCK_DATAS
						.get(oldCollectionName
								+ DataProviderService.MOCK_DATAS_KEY_CONNECTOR
								+ s);
				DataProviderService.MOCK_DATAS.put(collectionName
						+ DataProviderService.MOCK_DATAS_KEY_CONNECTOR + s,
						oMockInfo);
				DataProviderService.MOCK_DATAS.remove(oldCollectionName
						+ DataProviderService.MOCK_DATAS_KEY_CONNECTOR + s);
			}
			mockWebService.moveDataDirAndDelete(collectionName,
					oldCollectionName);
		} else if (!collectionName.equals(oldCollectionName)
				|| !mockName.equals(oldMockName)) {
			boolean flag = false;
			if (!DataProviderService.MOCK_COLLECTIONS
					.containsKey(collectionName)) {
				DataProviderService.MOCK_COLLECTIONS.put(collectionName,
			...
			if (DataProviderService.MOCK_COLLECTIONS.get(collectionName)
					.contains(mockName)) {
				if (flag) {
			...
			DataProviderService.MOCK_COLLECTIONS.get(collectionName).add(
					mockName);
			DataProviderService.MOCK_COLLECTIONS.get(oldCollectionName).remove(
					oldMockName);
			String okey = oldCollectionName
					+ DataProviderService.MOCK_DATAS_KEY_CONNECTOR
					+ oldMockName;
			...
			if (DataProviderService.MOCK_COLLECTIONS.get(oldCollectionName)
					.size() == 0) {
				DataProviderService.MOCK_COLLECTIONS.remove(oldCollectionName);
			}
			mockWebService.moveDataFile(collectionName, mockName,
					oldCollectionName, oldMockName);
		}
		ResponseUtil.ResponseInfo res = ResponseUtil.getSuccessResponse();
		res.setRetMsg("修改成功");
		return res;
	...
	@ResponseBody
	public ResponseUtil.ResponseInfo delete(String collectionName,
			String mockName) {
		if (StringUtils.isBlank(mockName)) {
			List<String> list = DataProviderService.MOCK_COLLECTIONS
					.get(collectionName);
			for (String s : list) {
				DataProviderService.MOCK_DATAS.remove(collectionName
						+ DataProviderService.MOCK_DATAS_KEY_CONNECTOR + s);
			}
			DataProviderService.MOCK_COLLECTIONS.remove(collectionName);
			mockWebService.deleteDataDir(collectionName);
		} else {
			DataProviderService.MOCK_COLLECTIONS.get(collectionName).remove(
					mockName);
	...
	@ResponseBody
	public Datagrid<MockRule> getMockRules(String collectionName,
			String mockName) {
		List<MockRule> list = new ArrayList<>();
		MockInfoDao rules = DataProviderService.MOCK_DATAS.get(collectionName
				+ DataProviderService.MOCK_DATAS_KEY_CONNECTOR + mockName);
		if (rules != null) {
			List<MockInfoDao.MockConditionInfo> conds = rules
	...
	@ResponseBody
	public ResponseUtil.ResponseInfo save(String collectionName,
			String mockName, MockInfoDao infos) {
		JSONArray conds = infos.getResCondition();
		JSONArray resValues = infos.getResValue();
		// 转为string 再转json
		String resValueStr = JSON.toJSONString(resValues);
		// ["{\"resultCode\":\"0\"","\"description\":\"OK\"}"] 需去掉开头及结尾自动加上的"
		if (resValueStr.startsWith("[\"{") && conds.size() == 1) {
			resValueStr = resValueStr.replace("[\"{", "[{")
					.replace("\",\"", ",").replace("}\"]", "}]")
					.replace("\\\"", "\"").replaceAll("\r|\n", "");
			;
		}
		logger.info("resValue:" + resValueStr);
		JSONArray newResValues = JSON.parseArray(resValueStr);

		logger.info("oldResValue size:" + resValues.size()
				+ ",newResValue size:" + newResValues.size());

		if (conds.size() != newResValues.size() || conds.size() == 0) {
			ResponseUtil.ResponseInfo res = ResponseUtil.getFailedResponse();
		...
		for (int i = 0; i < conds.size(); i++) {
			...
			condInfo.setResCondition(conds.getString(i));
			try {
				Object obj = JSON.parse(newResValues.getString(i));
				condInfo.setResValue(JSON.toJSONString(obj));
			} catch (Exception e) {
				condInfo.setResValue(newResValues.getString(i));
			}
		...
		DataProviderService.MOCK_DATAS.put(collectionName
				+ DataProviderService.MOCK_DATAS_KEY_CONNECTOR + mockName,
				infos);
		mockWebService.saveDataFile(collectionName, mockName,
				JSON.toJSONString(infos));
		ResponseUtil.ResponseInfo res = ResponseUtil.getSuccessResponse();
		res.setRetMsg("保存成功");
		return res;
	}
	...
	@ResponseBody
	public ResponseUtil.ResponseInfo setting(String mockDataDir,
			Integer mockDataCount, Integer mockDataRole) {
